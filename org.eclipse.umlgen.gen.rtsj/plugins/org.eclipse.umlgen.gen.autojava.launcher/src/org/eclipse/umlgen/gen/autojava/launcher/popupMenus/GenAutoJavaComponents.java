...
public abstract class GenAutoJavaComponents extends ActionDelegate implements IActionDelegate {
	...
	private void generate(IProgressMonitor monitor, AbstractAcceleoGenerator generator) {
		...
		monitor.worked(1);
		String generationID = org.eclipse.acceleo.engine.utils.AcceleoLaunchingUtil.computeUIProjectID("org.eclipse.umlgen.gen.autojava", getModuleQualifiedName(), generator.getModel().eResource().getURI().toString(), generator.getTargetFolder().getAbsolutePath(), new ArrayList<String>());
	...
	private File getFileFromPath(IFile model, String workspaceRelativePath) {
		...
		if (tempPath.isAbsolute()) {
			...
			path = model.getWorkspace().getRoot().getLocation().append(new Path(workspaceRelativePath)).toString();
		}
		return new File(path);
	}

	abstract protected Uml2autojava getGenerator(URI modelURI, String sDecorators, File target) throws IOException;
