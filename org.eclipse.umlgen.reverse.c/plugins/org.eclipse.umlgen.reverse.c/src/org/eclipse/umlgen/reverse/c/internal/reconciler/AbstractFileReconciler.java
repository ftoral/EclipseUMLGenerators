...
public abstract class AbstractFileReconciler implements IFileReconciler
{
    ...
    private CModelChangedEvent buildEvent(EventType eType, IASTEnumerationSpecifier specifier, IEnumeration enumeration)
    {
        String enumName = ModelUtil.computeAnonymousTypeName(enumeration.getTranslationUnit().getPath().removeFileExtension().lastSegment(), enumeration.getElementName(), specifier);
    ...
    private CModelChangedEvent buildEvent(EventType eType, IASTCompositeTypeSpecifier specifier, IStructure structure)
    {
        ...
        {
            // TODO Use a more detailed message to locate the problem : Unable to find if XXX in XXX is a UNION.
            Activator.log("Can't deteremine a UNION.", IStatus.WARNING, cme);
        }

        String typeName = ModelUtil.computeAnonymousTypeName(structure.getTranslationUnit().getPath().removeFileExtension().lastSegment(), structure.getElementName(), specifier);
        if (!isUnion)
        {
            if (eType == EventType.ADD)
            {
                IASTDeclaration[] declarations = specifier.getDeclarations(false);
                return StructAdded.builder().setDeclarations(declarations).currentName(typeName).translationUnit(structure.getTranslationUnit()).build();
            }
            else
            {
                return StructRemoved.builder().currentName(typeName).translationUnit(structure.getTranslationUnit()).build();
            }
        }
        else
        {
            IASTDeclaration[] declarations = specifier.getDeclarations(false);
            return UnionAdded.builder().setDeclarations(declarations).currentName(typeName).translationUnit(structure.getTranslationUnit()).build();
        }
    }
     ...
     * @param specifier The AST type specifier
     * @param type The typedef
    ...
    private CModelChangedEvent buildEvent(EventType eType, IASTEnumerationSpecifier specifier, ITypeDef type)
    {
        ...
        if (eType == EventType.ADD)
        {
            return TypeDefEnumerationAdded.builder().setRedefinedEnumeration(redefinedEnumName).setSource(specifier).currentName(type.getElementName()).translationUnit(type.getTranslationUnit()).build();
        }
        else
        {
    ...
    private CModelChangedEvent buildEvent(EventType eType, IASTSimpleDeclaration element, IVariableDeclaration variableDecl)
    {
        ...
        typeName = ModelUtil.computeAnonymousTypeName(variableDecl.getTranslationUnit().getPath().removeFileExtension().lastSegment(), typeName, element.getDeclSpecifier());
        ...
        try
        {
            if (eType == EventType.ADD)
            {
                event = VariableDeclarationAdded.builder().setConst(variableDecl.isConst()).setExtern(element.getRawSignature().contains("extern")).setVolatile(variableDecl.isVolatile()).setIsStatic(
                        variableDecl.isStatic()).setRegister(element.getRawSignature().contains("register")).currentType(typeName).currentName(varName).translationUnit(
                        variableDecl.getTranslationUnit()).build();
            }
            else
            {
                event = VariableDeclarationRemoved.builder().currentType(typeName).currentName(variableDecl.getElementName()).translationUnit(variableDecl.getTranslationUnit()).build();
            }
        }
    ...
    private CModelChangedEvent buildEvent(EventType eType, IASTSimpleDeclaration element, IVariable variable)
    {
        ...
        for (IASTDeclarator declarator : element.getDeclarators())
        {
            if (variable.getElementName().equals(declarator.getName().toString()))
            {
                ...
                else if (initializer != null && initializer.getChildren()[0] instanceof IASTInitializerClause)
                {
        ...
        String typeName = ASTUtil.computeType(element);
        typeName = ModelUtil.computeAnonymousTypeName(variable.getTranslationUnit().getPath().removeFileExtension().lastSegment(), typeName, element.getDeclSpecifier());
        ...
        try
        {
            if (eType == EventType.ADD)
            {
                event = VariableDeclarationAdded.builder().setConst(variable.isConst()).setExtern(element.getRawSignature().contains("extern")).setVolatile(variable.isVolatile()).setIsStatic(
                        variable.isStatic()).setRegister(element.getRawSignature().contains("register")).setInitializerExpression(initialisationExpression).currentType(typeName).currentName(varName).translationUnit(
                        variable.getTranslationUnit()).build();
            }
            else
            {
                event = VariableDeclarationRemoved.builder().currentType(typeName).currentName(variable.getElementName()).translationUnit(variable.getTranslationUnit()).build();
            }
        }
        catch (CModelException e)
        {
    ...
    private CModelChangedEvent buildEvent(EventType eType, IASTCompositeTypeSpecifier specifier, ITypeDef type)
    {
        ...
        if (eType == EventType.ADD)
        {
            String redefinedStructName = type.getTypeName().replaceFirst(BundleConstants.STRUCT_REGEXP, "").trim();
            return TypeDefStructAdded.builder().setRedefinedStruct(redefinedStructName).setSource(specifier).currentName(type.getElementName()).translationUnit(type.getTranslationUnit()).build();
        }
        else
        {
     ...
     * @return the built event
     */
    private CModelChangedEvent buildEvent(EventType eType, IASTElaboratedTypeSpecifier specifier, ITypeDef type)
    {
        ...
        if (eType == EventType.ADD)
        {
            ...
            {
                String redefinedEnumName = type.getTypeName().replaceFirst(BundleConstants.ENUM_REGEXP, "").trim();
                return TypeDefEnumerationAdded.builder().setRedefinedEnumeration(redefinedEnumName).setSource(specifier).currentName(type.getElementName()).translationUnit(type.getTranslationUnit()).build();
            }
        }
        else
    ...
    private CModelChangedEvent buildEvent(EventType eType, IASTPreprocessorIncludeStatement include, ITranslationUnit tu)
    {
     ...
     */
    public CModelChangedEvent addElement(IASTFunctionDeclarator iFunctionDeclarator, IFunctionDeclaration originalFunction)
    {
     ...
     *      org.eclipse.cdt.core.model.IFunctionDeclaration)
     */
    public CModelChangedEvent removeElement(IASTFunctionDeclarator iFunctionDeclarator, IFunctionDeclaration originalFunction)
    {
        return buildEvent(EventType.REMOVE, iFunctionDeclarator, originalFunction);
    }

    private CModelChangedEvent buildEvent(EventType eType, IASTFunctionDeclarator iFunctionDeclarator, IFunctionDeclaration originalFunction)
    {
        try
        {
            ...
            if (eType == EventType.ADD)
            {
                return FunctionDeclarationAdded.builder().isStatic(originalFunction.isStatic()).setReturnType(originalFunction.getReturnType()).setParameters(parameters).currentName(
                        originalFunction.getElementName()).translationUnit(originalFunction.getTranslationUnit()).build();
            }
            else
            {
                return FunctionDeclarationRemoved.builder().isStatic(originalFunction.isStatic()).setParameters(parameters).setReturnType(originalFunction.getReturnType()).currentName(
                        originalFunction.getElementName()).translationUnit(originalFunction.getTranslationUnit()).build();
            }
        }
    ...
    private CModelChangedEvent buildEvent(EventType eType, IASTPreprocessorMacroDefinition astMacro, ITranslationUnit unit)
    {
    ...
    public CModelChangedEvent addElement(IASTDeclarator iDeclarator, ITypeDef typeDef)
    {
        ...
        if (iDeclarator instanceof IASTArrayDeclarator)
        {
            ...
            event = TypeDefArrayAdded.builder().setDimensions(size).setRedefinedType(typeDef.getTypeName()).currentType(typeDef.getTypeName()).currentName(typeDef.getElementName()).translationUnit(
        ...
        else if (iDeclarator instanceof IASTFunctionDeclarator)
        {
            ...
            event = TypeDefFunctionDeclarationAdded.builder().currentName(typeDef.getElementName()).setReturnType(returnType).setParameters(parameters).translationUnit(typeDef.getTranslationUnit()).build();
        }
        else if (iDeclarator instanceof IASTDeclarator)
        {
            event = TypeDefAdded.builder().setRedefinedType(typeDef.getTypeName()).currentType(typeDef.getTypeName()).currentName(typeDef.getElementName()).translationUnit(
                    typeDef.getTranslationUnit()).build();
        }
        return event;
    }
    ...
    public CModelChangedEvent removeElement(IASTDeclarator iDeclarator, ITypeDef typeDef)
    {
        ...
        if (iDeclarator instanceof IASTArrayDeclarator)
        {
            ...
            event = TypeDefArrayRemoved.builder().setDimensions(dim).setRedefinedType(typeDef.getTypeName()).currentName(typeDef.getElementName()).translationUnit(typeDef.getTranslationUnit()).build();
        }
        else if (iDeclarator instanceof IASTFunctionDeclarator)
        {
            List<FunctionParameter> parameters = ASTUtil.collectParameterInformation(iDeclarator);
            ...
            // Remove the round brakets
            String returnType = typeDef.getTypeName().substring(0, typeDef.getTypeName().indexOf("("));
            event = TypeDefFunctionDeclarationRemoved.builder().currentName(typeDef.getElementName()).setReturnType(returnType).setParameters(parameters).translationUnit(typeDef.getTranslationUnit()).build();
        }
        else if (iDeclarator instanceof IASTDeclarator)
        {
            event = TypeDefRemoved.builder().setRedefinedType(typeDef.getTypeName()).currentName(typeDef.getElementName()).translationUnit(typeDef.getTranslationUnit()).build();
        }
        return event;
    }
}
