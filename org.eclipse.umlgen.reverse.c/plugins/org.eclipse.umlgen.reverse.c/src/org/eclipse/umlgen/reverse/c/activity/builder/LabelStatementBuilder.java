...
public class LabelStatementBuilder extends AbstractBuilder {
	public LabelStatementBuilder(UMLActivityBuilder activityBuilder,
			UMLActivityFactory factory, CommentBuilder commentBuilder) {
		super(activityBuilder, factory, commentBuilder);
	}

	public ActivityNodesPins buildLabelStatement(IASTLabelStatement stmt,
		...
		OpaqueAction labelAction = factory.createOpaqueAction(body,
				currentContext);
		commentBuilder.buildComment(labelAction, getCommentInfo(stmt));

		// Build the nodes for the nested statement
		ActivityNodesPins nestedNodes = activityBuilder.buildNodes(
				stmt.getNestedStatement(), currentContext);

		// Create the flow between the nodes
		factory.createControlFlow(labelAction, nestedNodes.getStartNode(),
				currentContext);
