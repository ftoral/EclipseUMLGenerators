/*******************************************************************************
 * Copyright (c) 2008, 2014 Obeo, CNES.
...
public class Uml2AutoJavaComponentProperties extends PropertyPage
{
    ...
    private void addFirstSection(Composite parent)
    {
        ...
        try
        {
            ...
            s = ((IResource) getElement()).getPersistentProperty(new QualifiedName("", "OUTPUT_PATH_AUTOJAVA_COMP"));
            ...
            s = ((IResource) getElement()).getPersistentProperty(new QualifiedName("", "VALIDATE_MODEL_AUTOJAVA_COMP"));
            if (s != null)
            {
            ...
            s = ((IResource) getElement()).getPersistentProperty(new QualifiedName("", "COMMUNICATION_PARAMETERING"));
            if ((s != null) && (s.compareTo("") != 0))
            {
    ...
    public boolean performOk()
    {
        ...
        try
        {
            ((IResource) getElement()).setPersistentProperty(new QualifiedName("", "VALIDATE_MODEL_AUTOJAVA_COMP"), boolToString(validationButton.getSelection()));
            ((IResource) getElement()).setPersistentProperty(new QualifiedName("", "OUTPUT_PATH_AUTOJAVA_COMP"), outputText.getText());
            ((IResource) getElement()).setPersistentProperty(new QualifiedName("", "COMMUNICATION_PARAMETERING"), comParamText.getText());
        }
        catch (CoreException e)
        {
            return false;
    ...
    private List<IResource> getResourcesFromProperties(String property) {
		...
		for (int i = 0; i < paths.length; i++) {
			IResource resource = ((IResource) getElement()).getWorkspace().getRoot().findMember(paths[i]);
    ...
    private class BrowseParamModelButtonListener implements MouseListener {
    	...
    	private class PathLabelProvider extends WorkbenchLabelProvider {
    		...
    		protected String decorateText(String input, Object element) {
    			if (element instanceof IResource) {
    				return super.decorateText(input, element) + " - " + ((IResource)element).getParent().getFullPath().makeRelative().toString();
    			} else {
        ...
        public void mouseDown(org.eclipse.swt.events.MouseEvent arg0) {
            ListSelectionDialog dialog = new ListSelectionDialog(getShell(), ((IResource) getElement()).getWorkspace().getRoot(), getFileProvider(), new PathLabelProvider(), "Select parametering model :");
            ...
            return new WorkbenchContentProvider() {
				...
				private List<Object> getResult(Object[] elements) throws CoreException {
					...
					for (int i = 0; i < elements.length; i++) {
						if (elements[i] instanceof IFile && (((IFile)elements[i]).getFileExtension().equals("asl") || ((IFile)elements[i]).getFileExtension().equals("eth"))) {
							result.add(elements[i]);
						} else if (elements[i] instanceof IContainer && ((IContainer)elements[i]).isAccessible() && !((IContainer)elements[i]).isHidden()){
