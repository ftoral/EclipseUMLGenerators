...
public class CompoundStatementBuilder extends AbstractBuilder {
	...
	public CompoundStatementBuilder(UMLActivityBuilder activityBuilder,
			UMLActivityFactory factory, CommentBuilder commentBuilder) {
		super(activityBuilder, factory, commentBuilder);
	}

	public ActivityNodesPins buildCompoundStatement(IASTCompoundStatement stmt,
		...
		if (stmt.getStatements().length == 0) {
			...
			for (int i = 0; i < statements.length; i++) {
				...
				ActivityNodesPins nodes = activityBuilder.buildNodes(childStmt,
						currentContext);
